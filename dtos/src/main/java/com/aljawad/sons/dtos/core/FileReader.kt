 ...
 */
package com.aljawad.sons.dtos.core

import android.graphics.Bitmap
import android.graphics.Rect
import android.util.Log
import com.aljawad.sons.dtos.model.FaceNetModel
import com.aljawad.sons.dtos.utils.BitmapUtils
...
//    private lateinit var callback: ProcessCallback
    ...
    fun feedTheData(data: ArrayList<Pair<String, Bitmap>>) {
        numImages = data.size
        this@FileReader.data = data
    }

    fun run(data: ArrayList<Pair<String, Bitmap>>): Flow<BaseState<Pair<ArrayList<Pair<String, FloatArray>>, Int>>> {
        return scanImage(data[imageCounter].first, data[imageCounter].second)
    }
...
//    }
    ...
    private fun scanImage(
        name: String,
        image: Bitmap
    ): Flow<BaseState<Pair<ArrayList<Pair<String, FloatArray>>, Int>>> = flow {

        Log.v("scanImage", "Called")
        ...
        val x = detector.process(inputImage)
        Thread.sleep(200)
        Log.v("detector.process", "Called")
        if (x.isSuccessful) {
            val faces = x.result
            Log.v("detector.process", "isSuccessful")
            if (faces.size != 0) {
//                coroutineScope.launch {
                ...
                if (imageCounter + 1 != numImages) {
                    imageCounter += 1
                    emitAll(scanImage(data[imageCounter].first, data[imageCounter].second))
                } else {
                    // Processing done, reset the file reader.
                    if (imageData.isNullOrEmpty()) {
                        emit(BaseState.NotDataFound<Pair<ArrayList<Pair<String, FloatArray>>, Int>>())
                    } else {
                        emit(
                            PoJoByIdStateLoadedSuccessfully(
                                Pair(
                                    imageData,
                ...
                Log.v("detector.process", "UnSuccessful")
